...
- 将grid size由`common::roundUp(bh.nMaxSeqLen, 32) *bh.nBatchSize* bh.nBeamWidthOut`减少为`common::roundUp(bh.nMaxSeqLen, PER_STEP) *bh.nBatchSize* (common::roundUp(bh.nBeamWidthOut, BEAMS_PER_BLOCK ) / BEAMS_PER_BLOCK)`。
...
**addBiasSoftMax：** 对原始Logits进行后处理，将其转化为概率分布（Softmax）或对数概率（Log-Probs），同时应用多种采样策略。
查看TensorRT-LLM的默认实现：
